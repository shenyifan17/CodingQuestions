...
# 
# When trying to find the first position of the ‘key’, 
# we can update end = middle - 1 to see if the key is present before middle.

# When trying to find the last position of the ‘key’, 
# we can update start = middle + 1 to see if the key is present after middle.
# In both cases, we will keep track of the last position where we found the ‘key’. 
# These positions will be the required range.

def find_range(arr, key):
    result = [-1, -1]
    result[0] = binary_search(arr, key, False)
    if result[0] != -1: ## no need to search, 
                        ## if 'key' is not present in the input array 
