...
we can use a modified version of the Binary Search to 
find the ‘key’ in an infinite sorted array.

The only issue with applying binary search in this problem 
is that we don’t know the bounds of the array. 
To handle this situation, 
we will first find the proper bounds of the array where we can perform a binary search.

An efficient way to find the proper bounds 
is to start at the beginning of the array 
with the bound’s size as ‘1’ and exponentially increase 
...
# In the first part, 
# we keep increasing the bound’s size exponentially 
# (double it every time) while searching for the proper bounds. 
# Therefore, this step will take O(logN) assuming that the array will 
# have maximum ‘N’ numbers. 
# In the second step, 
