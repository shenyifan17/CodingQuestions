...
def merge(intervals_a, intervals_b):
    ...
    while i < len(intervals_a) and j < len(intervals_b):
        ...
        # i for interval_a, j for interval_b

        ## check if intervals overlap 
        ## and intervals_a[i]'s start time lies within 
        ## the other intervals_b[j]
        a_overlaps_b = (intervals_a[i][start] >= intervals_b[j][start]) and \
                       (intervals_a[i][start] <= intervals_b[j][end])
        
        ## check if intervals overlap and 
        ## intervals_a[j]'s start time lies within 
        ## the other intervals_b[i]
...
# where ‘N’ and ‘M’ are the total number of intervals 
# in the input arrays respectively.
# 
# Space complexity
# Ignoring the space needed for the result list, 
