...
# This problem follows the Two Pointers pattern and shares similarities 
# with Triplet Sum to Zero. The only difference is that, 
# in this problem, we need to find the triplets whose sum is less than the 
# given target. 
# To meet the condition i != j != k 
# we need to make sure that each number is not used more than once.
# 
# Following a similar approach, 
# first we can sort the array and then iterate through it, 
# taking one number at a time. 
# Let’s say during our iteration we are at number ‘X’, 
# so we need to find ‘Y’ and ‘Z’ such that X + Y + Z < targetX+Y+Z<target. 
# At this stage,
...
def search_pair(arr, target_sum, first):
    ...
    while (left < right):
        if arr[left] + arr[right] < target_sum: # found the triplet
            ...
            # arr[right] by any number between left and right to get a sum less 
            # than the target sum 
            count += right - left 
            left += 1 
        else: 
            right -= 1 # need to pair with a smaller sum 
    return count 
...
## Similar Question: 
# instead of returning count, returna all triplets 
def triplet_with_smaller_sum_(arr, target):
    arr.sort() 
    triplets = [] 
    for i in range(len(arr) - 2):
        search_pair_(arr, target - arr[i], i, triplets)
    return triplets

def search_pair_(arr, target_sum, first, triplets):
    left = first + 1 
    right = len(arr) - 1
    while (left < right):
        if arr[left] + arr[right] < target_sum: # found the triplet 
            # since arr[right] >= arr[left], we can replace arr[right] by 
...
​which is asymptotically equivalent to O(N^3)j
