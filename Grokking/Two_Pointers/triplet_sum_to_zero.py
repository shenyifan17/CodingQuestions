...
# Solution: 
# This problem follows the Two Pointers pattern and shares 
# similarities with Pair with Target Sum. 
# A couple of differences are that the input array is not sorted 
# and instead of a pair we need to find triplets with a target sum of zer# 
# To follow a similar approach, 
# first, we will sort the array and then iterate through it 
# taking one number at a time. 
# Let’s say during our iteration we are at number ‘X’, 
# so we need to find ‘Y’ and ‘Z’ such that X+Y+Z==0. 
# At this stage, 
# our problem translates into finding a pair 
# whose sum is equal to “−X” (as from the above equation Y + Z == -X
# Another difference from Pair with Target Sum is that we need 
# to find all the unique triplets. 
# To handle this, 
...
def search_triplets(arr):
    ...
    for i in range(len(arr)):
        if (i > 0) and (arr[i] == arr[i-1]): # skip same element to aviod duplicate triplets
            continue 
...
def search_pair(arr, target_sum, left, triplets):
    ...
    while (left < right):
        ...
        if current_sum == target_sum: # found the triplet
            ...
            right -= 1 # we meed a pair with smaller sum 
...
As we are calling search_pair for every number in the input array, this means that 
overall search_triplets will take O(N*logN + N^2), which asymptotically to O(N^2)
